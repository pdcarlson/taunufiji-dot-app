...
export const TasksService = {
  ...
  async createTask(data: CreateAssignmentDTO) {
    return await db.createDocument(DB_ID, COLLECTIONS.ASSIGNMENTS, ID.unique(), {
        ...data,
  ...
  async claimTask(taskId: string, profileId: string) {
      ...
      return await db.updateDocument(DB_ID, COLLECTIONS.ASSIGNMENTS, taskId, updates);
  },
  ...
  async adminReassign(taskId: string, profileId: string | null) {
      ...
      if (task.schedule_id) {
          try {
              const schedule = await db.getDocument(DB_ID, COLLECTIONS.SCHEDULES, task.schedule_id);
  ...
  async verifyTask(taskId: string) {
      ...
      const task = await db.updateDocument(DB_ID, COLLECTIONS.ASSIGNMENTS, taskId, {
          status: "approved"
      });
   ...
   */
  async triggerNextInstance(scheduleId: string, previousTask: any) {
      const schedule = await db.getDocument(DB_ID, COLLECTIONS.SCHEDULES, scheduleId);
      ...
      const prevDue = previousTask.due_at ? new Date(previousTask.due_at) : completedAt;
      
      // Next Due = Previous Due + Interval (Keep the cycle)
      const nextDue = new Date(prevDue.getTime() + (intervalDays * 24 * 60 * 60 * 1000));
      ...
      let unlockAt = cycleStart.getTime() > cooldownEnd.getTime() ? cycleStart : cooldownEnd;
  ...
  async createSchedule(data: any) {
      const schedule = await db.createDocument(DB_ID, COLLECTIONS.SCHEDULES, ID.unique(), {
          ...data,
          active: true,
          last_generated_at: new Date().toISOString()
      });
      ...
      await this.createTask({
          ...
          due_at: new Date(Date.now() + (parseInt(schedule.recurrence_rule) * 24 * 60 * 60 * 1000)).toISOString(),
          status: "open"
      });
  ...
  async runCron() {
      ...
      const unlockPromises = lockedTasks.documents.map(doc => 
          db.updateDocument(DB_ID, COLLECTIONS.ASSIGNMENTS, doc.$id, { status: "open" })
      );

      // 2. Expire Bounties
      // Query: status=open AND type=bounty AND expires_at <= now
      const expiredBounties = await db.listDocuments(DB_ID, COLLECTIONS.ASSIGNMENTS, [
          Query.equal("status", "open"),
      ...
      const expirePromises = expiredBounties.documents.map(doc => 
          db.updateDocument(DB_ID, COLLECTIONS.ASSIGNMENTS, doc.$id, { status: "expired" })
      );
      ...
      const overdueDuties = await db.listDocuments(DB_ID, COLLECTIONS.ASSIGNMENTS, [
          Query.equal("status", "open"),
      ...
      const finePromises = overdueDuties.documents.map(async (doc) => {
          ...
          if (doc.schedule_id) {
             ...
             } catch(e) { console.error("Recurrence failed for fined task", e); }
          }
      });
